...
/// Create and initialize a Kalman filter (like the OpenCV example)
/// @param dynam_params Number of state dimensions
/// @param measure_params Number of measurement dimensions
/// @param control_params Number of control dimensions (default 0)
/// @return Initialized cv::KalmanFilter ready to use
...
public:
  Sesenta(Context &ctx_)
    ...
    // Set default matrices like the OpenCV example
    ...
    // Measurement matrix - we only measure position (direction)
    kf.measurementMatrix = (cv::Mat_<float>(1, 2) << 1, 0);
    // Process noise - how much we expect the direction to change
    cv::setIdentity(kf.processNoiseCov, cv::Scalar::all(1));  // tune this
    // Measurement noise - how noisy our direction readings are (high = more smoothing)
    cv::setIdentity(kf.measurementNoiseCov, cv::Scalar::all(25));  // tune this
    // Initial error covariance
    cv::setIdentity(kf.errorCovPost, cv::Scalar::all(1));
    // Initial state (start at direction 15, velocity 0)
...
//  void create_kalman_filter(int32_t dynam_params, int32_t measure_params,
// auto create_kalman_filter(int32_t dynam_params, int32_t measure_params,
//                                         int32_t control_params = 0) {
//     cv::KalmanFilter kf(dynam_params, measure_params, control_params, CV_32F);
...
private:
  ...
  cv::KalmanFilter kf;  // Use braces with actual values
  // cv::KalmanFilter kf{2, 1, 0, CV_32F};  // Use braces with actual values
  ...
  //  28, 26, 24, 22, 20, 18, 16, 14, 12, 10, 8, 6, 4, 2, 59
  // };
  static constexpr std::array<uint8_t, 30> M_DATA_TO_MIC = {
