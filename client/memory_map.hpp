...
class Memory
{
  public:
   ...
   // Read a value of a std::array at a specific index (offset defined at compile-time)
    template<typename T, size_t N, uint32_t offset = 0>
    T read_array_value_at_index(uint32_t index, uint32_t block_idx = 0) {
        static_assert(offset + sizeof(T) * (N - 1) < (mem::get_range(id) * mem::get_n_blocks(id)), "Invalid offset");
        // static_assert(index < N, "Index out of bounds");
        static_assert(mem::is_readable(id), "Not readable");

        T* p = get_ptr<T, offset>(block_idx);
        return *(p + index); // Dereference the pointer with the index to get the specific element
    }
 

    // Read the first value of a std::array (offset defined at compile-time)
    template<typename T, size_t N, uint32_t offset = 0>
    T read_first_array_value(uint32_t block_idx = 0) {
        static_assert(offset < mem::get_range(id), "Invalid offset");
        static_assert(sizeof(T) <= (mem::get_range(id) * mem::get_n_blocks(id)), "Invalid size for first element");
        static_assert(mem::is_readable(id), "Not readable");

        T* p = get_ptr<T, offset>(block_idx);
        return *p; // Dereference the pointer to get the first element
    }


    // Read a std::array (offset defined at compile-time)
    // template<typename T, size_t N, uint64_t offset = 0>
    // std::array<T, N>& read_array(uint64_t block_idx = 0) {
    //     static_assert(offset + sizeof(T) * (N - 1) < (mem::get_range(id) * mem::get_n_blocks(id)), "Invalid offset");
    //     static_assert(mem::is_readable(id), "Not readable");
