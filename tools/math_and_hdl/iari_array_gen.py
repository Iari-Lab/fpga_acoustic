...
class MicArrayTool:
    ...
    def plot_microphones(self, selected_mics, save_path="output/microphone_array.png", 
        ...
        if show_svg_background:
            ...
            if result[0] is not None:
                ...
                ax.imshow(svg_img, extent=extent, aspect='auto', alpha=0.85, zorder=0)
    ...
    def generate_xdc(self, selected_mics, output_path="output/ports.xdc"):
        ...
        xdc_lines = [
            ...
            f"# Selection: {len(selected_mics)} microphones",
            f"# Microphone indices: {selected_mics}",
            ""
        ]
        
        selected_df = self.df.iloc[selected_mics] if selected_mics else pd.DataFrame()
        
        for seq_idx, (orig_idx, row) in enumerate(selected_df.iterrows()):
            mic_name = row['mic']
            d_pin = row['D_pin']
            io_pin = row['io_pin']
            mcu_pin = row['MCU_Pin']
            
            xdc_lines.append(f"# {mic_name} - {d_pin} -> M_DATA[{seq_idx}]")
            xdc_lines.append(f"# IO Pin: {io_pin}")
            xdc_lines.append(f"set_property PACKAGE_PIN {mcu_pin} [get_ports M_DATA[{seq_idx}]]")
            xdc_lines.append("")
    ...
    def calculate_delays(self, selected_mics):
        ...
        mic_positions = selected_df[['x', 'y']].values / 1000.0  # Convert to meters
    ...
    def generate_verilog(self, selected_mics, output_path="output/delay_module.v"):
        ...
        v.append(f"          .delay(selected_delays[i*{delay_bits} +: {delay_bits}]),")
    ...
    def generate_delay_config(self, selected_mics, output_path="output/delay_config.v"):
        ...
        v.append(f"    parameter DELAY_SELECT = 0")
        v.append(f")(")
        v.append(f"    output wire [{total_bits-1}:0] delays  // {num_mics} x {delay_bits}-bit delays packed")
        v.append(f");")
        v.append(f"")
        
        # Generate pack_delays function
        d_inputs = ", ".join([f"d{i}" for i in range(num_mics)])
        v.append(f"  // Function to pack {num_mics} {delay_bits}-bit delays into {total_bits} bits")
        v.append(f"  function [{total_bits-1}:0] pack_delays;")
        
        # Split inputs into multiple lines if many mics
        half = num_mics // 2
        v.append(f"    input [{delay_bits-1}:0] " + ", ".join([f"d{i}" for i in range(half)]) + ";")
        v.append(f"    input [{delay_bits-1}:0] " + ", ".join([f"d{i}" for i in range(half, num_mics)]) + ";")
        v.append(f"    begin")
        ...
        v.append(f"  // Delay configurations for all {num_mics} source directions")
        ...
        for case_idx, source_mic_id in enumerate(mic_ids):
            ...
            v.append(f"  localparam [{total_bits-1}:0] DELAY_CONFIG_{case_idx:<2} = pack_delays({delay_str});  // Source M{source_mic_id}")
        ...
        for case_idx in range(num_mics):
            if case_idx < num_mics - 1:
                v.append(f"    (DELAY_SELECT == {case_idx:<2}) ? DELAY_CONFIG_{case_idx:<2} :")
            else:
                v.append(f"    (DELAY_SELECT == {case_idx:<2}) ? DELAY_CONFIG_{case_idx:<2} :")
    ...
    def generate_delay_bank(self, selected_mics, output_path="output/delay_bank.v"):
        ...
        v.append(f"    parameter NUM_CONFIGS = {num_mics},   // Number of delay configurations (directions)")
        v.append(f"    parameter NUM_CHANNELS = {num_mics}   // Number of microphone channels")
        v.append(f")(")
        v.append(f"    input wire clk,")
        v.append(f"    input wire rst,")
        v.append(f"    input wire pcm_valid,")
        v.append(f"    input wire [NUM_CHANNELS*16-1:0] pcm_data,                      // Input: NUM_CHANNELS * 16 bits")
        v.append(f"    output wire [NUM_CONFIGS*NUM_CHANNELS*16-1:0] delayed_data      // Output: NUM_CONFIGS * NUM_CHANNELS * 16 bits")
        v.append(f");")
        v.append(f"")
        ...
        v.append(f"          .delayed_pcm_data(delayed_data[i*NUM_CHANNELS*16 +: NUM_CHANNELS*16])")
        v.append(f"      );")
        v.append(f"    end")
        v.append(f"  endgenerate")
        v.append(f"")
    ...
    def generate_all(self, mics, output_dir="output"):
        ...
        delay_line_path = self.generate_delay_line(mics, f"{output_dir}/delay_line.v")
        
        # 2. delay_config (delay matrix)
        delay_config_path = self.generate_delay_config(mics, f"{output_dir}/delay_config.v")
        
        # 3. delay_module (uses delay_config and delay_line)
        delay_module_path = self.generate_verilog(mics, f"{output_dir}/delay_module.v")
        
        # 4. delay_bank (instantiates multiple delay_modules)
        delay_bank_path = self.generate_delay_bank(mics, f"{output_dir}/delay_bank.v")
